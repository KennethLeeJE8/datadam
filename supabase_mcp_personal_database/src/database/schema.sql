...
CREATE TABLE IF NOT EXISTS data_field_definitions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  field_name TEXT NOT NULL UNIQUE,
  data_type TEXT NOT NULL CHECK (data_type IN ('string', 'number', 'date', 'json', 'encrypted')),
...
CREATE INDEX IF NOT EXISTS idx_data_field_definitions_field_name ON data_field_definitions(field_name);
CREATE INDEX IF NOT EXISTS idx_data_field_definitions_data_type ON data_field_definitions(data_type);
...
ALTER TABLE data_field_definitions ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "service_role_full_access_field_definitions" ON data_field_definitions
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
-- Data field definitions policies (read-only for users, managed by admins)
CREATE POLICY "users_can_read_field_definitions" ON data_field_definitions
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "users_can_insert_field_definitions" ON data_field_definitions
  FOR INSERT TO authenticated
  WITH CHECK (true);

CREATE POLICY "users_can_update_field_definitions" ON data_field_definitions
  FOR UPDATE TO authenticated
  USING (true);
...
-- Create function to get data type statistics
CREATE OR REPLACE FUNCTION get_data_type_stats()
RETURNS TABLE (
  data_type TEXT,
  classification TEXT,
  record_count BIGINT,
...
BEGIN
  ...
  GROUP BY pd.data_type, pd.classification
...
BEGIN
  ...
  WHERE pd.user_id = p_user_id
    ...
    AND (p_data_types IS NULL OR pd.data_type = ANY(p_data_types))
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'field_definitions', (
      ...
      FROM data_field_definitions fd
    ),
    'audit_logs', (
...
INSERT INTO data_field_definitions (
  field_name,
  data_type,
  validation_rules,
  is_required
) VALUES 
  ('sample_email', 'string', '{"format": "email"}', false),
  ('sample_age', 'number', '{"min": 0, "max": 150}', false)
